...
public static class UnityExtentions
{
    ...
    public static Vector3 RotateAroundPivot(this Vector3 point, Vector3 pivot, Vector3 angles) => Quaternion.Euler(angles) * (point - pivot) + pivot;

    public static Vector2 RotateAroundPivot(this Vector2 point, float angles) => Quaternion.Euler(0,0,angles) * point + Vector3.forward;
    ...
    public static Vector2 RotateLocal(this Vector2 source, float angle)
    {
        return (source.AsAngle() + angle).AsVector();
    }
...
public enum DeltaTimeType { deltaTime, unscaledDeltaTime, fixedDeltaTime, fixedUnscaledDeltaTime }
